...
class policy_transformer_set_fusion(nn.Module):
    ...
    def __init__(self, d_model: int = 128, n_heads: int = 8, dropout: float = 0.0,
    ...
    def forward(self, relational_feature, temporal_feature_short, temporal_feature_long, holding, mask=None):
        ...
        r = self.proj_r(relational_feature) + self.type_emb[0]
        s = self.proj_s(temporal_feature_short) + self.type_emb[1]
        l = self.proj_l(temporal_feature_long) + self.type_emb[2]
