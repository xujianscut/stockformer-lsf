...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # Implements L_ES = (1/(N(N-1))) * sum_{i!=j} <grad_a Q_i, grad_a Q_j> / (||grad_a Q_i|| ||grad_a Q_j||)
            ...
            # Q-Ensemble pessimistic actor objective: maximize mean - beta * std
            ...
            pess_qf_pi = q_pi_mean - self.q_pess_beta * q_pi_std
            actor_loss = (ent_coef * log_prob - pess_qf_pi).mean() + alpha * th.abs(th.mean(th.sum(replay_data.actions, dim=-1))-1)
