...
from MySAC.SAC.flow_actor import FlowSACPolicy
import pdb
...
class SAC(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        critic_alpha=1,
        actor_alpha=0,
        flow_bc_coef: float = 0.1,
        flow_noise_std: float = 1.0,
        q_pess_beta: float = 0.3,
        edac_eta: float = 1.0,
    ):
        ...
        self.flow_bc_coef = flow_bc_coef
        self.flow_noise_std = flow_noise_std
        self.q_pess_beta = q_pess_beta
        self.edac_eta = edac_eta
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                # Q-Ensemble pessimistic target: mean - beta*std across N critics
                next_q_all = th.cat(self.critic_target(self.critic_transformer(next_state, next_temporal_feature_short, next_temporal_feature_long, next_holding_stocks), next_actions), dim=1)
                next_q_mean = next_q_all.mean(dim=1, keepdim=True)
                next_q_std = next_q_all.std(dim=1, keepdim=True) if next_q_all.shape[1] > 1 else th.zeros_like(next_q_mean)
                next_q_values = next_q_mean - self.q_pess_beta * next_q_std
                q_std_track.append(next_q_std.mean().item())
                # add entropy term (zero for flow actor since log_prob = 0)
            ...
            # EDAC diversity loss: encourage orthogonal action-gradients across critics
            edac_term = th.zeros((), device=critic_loss.device)
            n_q = len(current_q_values)
            if self.edac_eta > 0 and n_q > 1:
                critic_feat = self.critic_transformer(state, temporal_feature_short, temporal_feature_long, holding_stocks).detach()
                acts_with_grad = replay_data.actions.detach().clone().requires_grad_(True)
                q_each = self.critic(critic_feat, acts_with_grad)
                grads = []
                for q_i in q_each:
                    g = th.autograd.grad(q_i.sum(), acts_with_grad, create_graph=True, retain_graph=True)[0]
                    grads.append(g)
                g_stack = th.stack(grads, dim=1)
                g_norm = g_stack / (g_stack.norm(dim=-1, keepdim=True) + 1e-8)
                sim = th.einsum('bnd,bmd->bnm', g_norm, g_norm)
                eye = th.eye(n_q, device=sim.device).unsqueeze(0)
                off_diag = sim * (1 - eye)
                edac_term = off_diag.sum(dim=(1, 2)).mean() / (n_q * (n_q - 1))
                edac_losses.append(edac_term.item())
                critic_loss = critic_loss + self.edac_eta * edac_term
            ...
            # Pessimistic actor objective: mean - beta * std
            q_pi_mean = q_values_pi.mean(dim=1, keepdim=True)
            q_pi_std = q_values_pi.std(dim=1, keepdim=True) if q_values_pi.shape[1] > 1 else th.zeros_like(q_pi_mean)
            min_qf_pi = q_pi_mean - self.q_pess_beta * q_pi_std

            # Flow-matching BC loss: W2-bound anchor toward replay actions.
            # For (s, a) in buffer, sample a0 ~ N(0, sigma^2 I), t ~ U(0,1),
            # a_t = (1-t)*a0 + t*a; target velocity u_t = a - a0.
            # Loss = E ||v_theta(features, a_t, t) - (a - a0)||^2
            actor_feat = self.actor_transformer(state.detach(), temporal_feature_short, temporal_feature_long, holding_stocks)
            actor_obs_flat = self.actor.extract_features(actor_feat)
            B = actor_obs_flat.shape[0]
            A = self.actor.action_dim
            a_replay = replay_data.actions
            a0_bc = th.randn(B, A, device=actor_obs_flat.device) * self.actor.noise_std
            t_bc = th.rand(B, 1, device=actor_obs_flat.device)
            a_t = (1.0 - t_bc) * a0_bc + t_bc * a_replay
            v_pred = self.actor._velocity(actor_obs_flat, a_t, t_bc)
            flow_bc_loss = ((v_pred - (a_replay - a0_bc)) ** 2).mean()
            flow_bc_losses.append(flow_bc_loss.item())

            actor_loss = (ent_coef * log_prob - min_qf_pi).mean() + self.flow_bc_coef * flow_bc_loss + alpha * th.abs(th.mean(th.sum(replay_data.actions, dim=-1))-1)
        ...
        if len(flow_bc_losses) > 0:
            self.logger.record("train/flow_bc_loss", np.mean(flow_bc_losses))
        if len(edac_losses) > 0:
            self.logger.record("train/edac_loss", np.mean(edac_losses))
        if len(q_std_track) > 0:
            self.logger.record("train/q_target_std", np.mean(q_std_track))
