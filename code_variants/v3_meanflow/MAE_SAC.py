...
from MySAC.SAC.meanflow_actor import MeanFlowSACPolicy, MeanFlowActor
import pdb
...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # MeanFlow Identity BC loss: u_theta(a_t, r, t; s) regresses to u_tgt = v - (t-r)*du/dt
            # OFQL (ICLR'26) / MeanFlowQL (AAAI'26) parametrization. v = eps - a (constant).
            # du/dt = v * d_{a_t} u + d_t u  (chain rule, since da_t/dt = v).
            ...
            device = actor_obs_flat.device

            a_replay = replay_data.actions
            eps_bc = th.randn(B, A, device=device) * self.actor.noise_std
            t_bc = th.rand(B, 1, device=device)
            # Flow ratio lambda: with prob lambda, r=t (instantaneous case), else r~U(0,t).
            mask_inst = (th.rand(B, 1, device=device) < self.flow_ratio).float()
            r_rand = t_bc * th.rand(B, 1, device=device)
            r_bc = mask_inst * t_bc + (1.0 - mask_inst) * r_rand

            a_t_bc = (1.0 - t_bc) * a_replay + t_bc * eps_bc
            v_inst = eps_bc - a_replay  # instantaneous velocity along the linear path

            # u_theta value at (a_t, r, t) — gradient flows to actor params.
            u_out = self.actor.u_velocity(actor_obs_flat, a_t_bc, r_bc, t_bc)

            # du/dt via JVP on (a_t, t), with features and r frozen.
            features_sg = actor_obs_flat.detach()
            r_sg = r_bc.detach()
            def _u_fn(a_t_in, t_in):
                return self.actor.u_velocity(features_sg, a_t_in, r_sg, t_in)
            try:
                _, du_dt = th.func.jvp(_u_fn, (a_t_bc.detach(), t_bc.detach()),
                                       (v_inst.detach(), th.ones_like(t_bc)))
            except Exception:
                # Fallback: no MeanFlow Identity, regress to instantaneous velocity only.
                du_dt = th.zeros_like(v_inst)
            u_tgt = (v_inst - (t_bc - r_bc) * du_dt).detach()
            flow_bc_loss = ((u_out - u_tgt) ** 2).mean()
